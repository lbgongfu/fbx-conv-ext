...
class FbxConv {
	public:
		...
		bool execute(Settings * const &settings) {
			...
			modeldata::ModelExtras *modelExtras = new modeldata::ModelExtras();
			if (load(settings, model, modelExtras)) {
				if (settings->verbose)
					info(model);
				if (save(settings, model, modelExtras))
					result = true;
			}
			delete model;
			return result;
		...
		bool load(Settings * const &settings, modeldata::Model *model, modeldata::ModelExtras *modelExtras) {
			...
			if (!result)
				...
				result = reader->convert(model, modelExtras);
		...
		////add parameter "modelExtras"(update by lbgongfu)
		bool save(Settings * const &settings, modeldata::Model *model, modeldata::ModelExtras *modelExtras) {
			bool result = false;
			std::ofstream myfile;
			myfile.open (settings->outFile.c_str(), std::ios::binary);

			//(update by lbgongfu)
			std::ofstream modelExtrasFile;
			modelExtrasFile.open(settings->modelExtrasFile.c_str(), std::ios::binary);

			json::BaseJSONWriter *jsonWriter = 0;
			json::BaseJSONWriter *jsonWriter1 = 0;
			switch(settings->outType) {
			case FILETYPE_G3DB: 
				log->status(log::sExportToG3DB, settings->outFile.c_str());
				jsonWriter = new json::UBJSONWriter(myfile);
				//(update by lbgongfu)
				jsonWriter1 = new json::UBJSONWriter(modelExtrasFile);
				break;
			case FILETYPE_G3DJ: 
				log->status(log::sExportToG3DJ, settings->outFile.c_str());
				jsonWriter = new json::JSONWriter(myfile);
				//(update by lbgongfu)
				jsonWriter1 = new json::JSONWriter(modelExtrasFile);
				break;
			default: 
