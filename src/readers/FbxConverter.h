...
namespace readers {
	...
	public:
		...
		ModelExtrasHelper *modelExtrasHelper;
		...
		bool load(Settings *settings) {
			...
			//fetch all camera nodes(add by lbgongfu)
			if (scene)
			{
				modelExtrasHelper = new ModelExtrasHelper(scene);
				modelExtrasHelper->FetchModelExtras();
			}
			//
			if (scene)
				checkNodes();
		...
		virtual bool convert(Model * const &model, ModelExtras * const &modelExtras) {
			if (!scene) {
			...
			//update by lbgongfu
			modelExtrasHelper->AddModelExtras(modelExtras);
			addMesh(model);
			addNode(model);
