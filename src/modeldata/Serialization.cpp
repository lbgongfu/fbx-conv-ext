...
void CameraData::serialize(json::BaseJSONWriter &writer) const
{
	writer.obj(7);
	writer << "position" = position;
	writer << "lookAt" = lookAt;
	writer << "up" = up;
	writer << "fieldOfView" = fieldOfView;
	writer << "aspectRatio" = aspectRatio;
	writer << "near" = near;
	writer << "far" = far;
	writer.end();
}

void ModelExtras::serialize(json::BaseJSONWriter &writer) const
{
	writer.obj(4);
	writer << "version" = mVersion;
	writer << "ambientColor" = mAmbientColor;
	writer << "cameraDatas" = mCameraDatas;
	writer << "lightDatas" = mLightDatas;
	writer.end();
}

void LightData::serialize(json::BaseJSONWriter &writer) const
{
	writer.obj(7);
	writer << "type" = mType;
	writer << "color" = mColor;
	writer << "direction" = mDirection;
	writer << "position" = mPosition;
	writer << "intensity" = mIntensity;
	writer << "cutOffAngle" = mCutoffAngle;
	writer << "exponent" = mExponent;
	writer.end();
}
